...
@router.get("/lanes")
async def lanes(biz_type: str = "海运", db: AsyncSession = Depends(get_db)):
    rows = (await db.execute(
        select(CostLane).where(CostLane.biz_type == biz_type).order_by(CostLane.id)
    )).scalars().all()
...
@router.get("/trend")
async def trend(biz_type: str = "海运", lane: str | None = None,
                db: AsyncSession = Depends(get_db)):
    """每条线路的目标成本时间序列（喂折线图）。"""
    lq = select(CostLane).where(CostLane.biz_type == biz_type)
    if lane:
...
@router.get("/current")
async def current(biz_type: str = "海运", as_of: str | None = None,
                  db: AsyncSession = Depends(get_db)):
...
async def last_period(biz_type: str = "海运", db: AsyncSession = Depends(get_db)):
