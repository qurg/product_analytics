...
const subLabel = computed(() => (isCB.value ? '运输类型' : (isSP.value ? '线路' : '')))
const hasSub = computed(() => isCB.value || isSP.value)
...
function params() {
  ...
  if (isSP.value) p.region = sub.value
  return p
}
...
<template>
  ...
  <div class="page" style="max-width:none">
    <div class="card" v-if="showEntry" style="margin-bottom:12px;border:1px solid var(--blue)">
      ...
      <table>
        <thead><tr><th>{{ isCB ? '区域' : (isSP ? 'PD' : '线路') }}</th><th>上期价</th><th>本期目标成本</th><th></th></tr></thead>
        <tbody>
    ...
    <div class="card">
      ...
      <table>
        <thead><tr>
          <th>{{ isCB ? '区域' : (isSP ? '线路' : '线路') }}</th>
          <th v-if="isSP">PD</th><th v-if="isSP">国家</th>
          <th v-if="!isSP">目的港</th><th v-if="!isCB && !isSP">船东</th>
          <th>当前目标成本</th><th>生效区间</th><th>环比上期</th>
          <th v-if="!isCB && !isSP">起运港费</th>
          <th v-if="isSP">揽收/库内/清关</th>
        </tr></thead>
        <tbody>
          <tr v-for="r in current.rows" :key="(r.lane||'')+(r.dest_ports||'')">
            <td><b>{{ isSP ? sub : r.lane }}</b></td>
            <td v-if="isSP" style="font-size:12px">{{ r.lane }}</td>
